...
`demos/research-pipeline/weather/sources/selected-papers.json` and produces one structured
markdown file per paper in `demos/research-pipeline/weather/knowledge/`.
...
Read `demos/research-pipeline/weather/sources/selected-papers.json`. Verify it exists and
...
2. **Abstract + TLDR** — from the Semantic Scholar API using the DOI:
   ```bash
   python3 demos/research-pipeline/weather/scripts/semantic_scholar.py "<paper title>" --limit 1
   ```
...
Save each paper as `demos/research-pipeline/weather/knowledge/<citation-key>.md` where
...
Save `demos/research-pipeline/weather/knowledge/index.md`:
...
- Path to `demos/research-pipeline/weather/knowledge/` and `index.md`
